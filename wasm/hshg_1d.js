...
const binary = require("node:fs").readFileSync("./hshg_1d.wasm");
...
class HSHG_1D {
    ...
    #handle_entity_update(id, entity) {
        const idx = this.#head + id * 7 + 5;

        const x = this.#f32_memory[idx + 0];
        const r = this.#f32_memory[idx + 1];
        ...
        if(entity.r != r) {
            this.#f32_memory[idx + 1] = entity.r;
    ...
    insert(entity) {
        ...
        this.#exports.insert(entity.x, entity.r, idx);
    }
     ...
     */
    set update(callback) {
        HSHG_1D.#sanitize_callback(callback, 1);
    ...
    set collide(callback) {
        HSHG_1D.#sanitize_callback(callback, 2);
     ...
     * @param {number} max_x
     */
    #do_query(min_x, max_x) {
        this.#exports.query(min_x, max_x);
    }
