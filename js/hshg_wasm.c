...
uint32_t new_balls[AGENTS_NUM];
uint32_t new_balls_idx = 0;
...
EM_BOOL tick(double nil1, void* nil2) {
  ...
  hshg_update(hshg);
  for(uint32_t j = 0; j < new_balls_idx; ++j) {
    const uint32_t ref = new_balls[j];
...
int main() {
  ...
  hshg = hshg_create(CELLS_SIDE, CELL_SIZE);
  assert(hshg);
  hshg->update = update;
